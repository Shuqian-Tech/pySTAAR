...
- STAAR-51: Expand distinct non-`example` conditional parity beyond sparse related STAAR to at least one additional conditional family (for example `staar_related_dense_glmmkin_cond` and/or `indiv_score_related_*_glmmkin_cond`). (Resolved; extracted `baselines/related_dense_glmmkin_cond_nonexample601_sentinels.json` from `../STAAR` and added parity scenario `specs/staar_related_dense_glmmkin_cond_nonexample601.yaml`; parity now `52 passed`)
- STAAR-52: Extend distinct non-`example` conditional parity into conditional individual-score related workflows (for example `indiv_score_related_sparse_glmmkin_cond`). (Resolved; extracted `baselines/indiv_score_related_sparse_glmmkin_cond_nonexample601_sentinels.json` from `../STAAR` and added parity scenario `specs/indiv_score_related_sparse_glmmkin_cond_nonexample601.yaml`; parity now `53 passed`)
- STAAR-53: Expand non-`example` conditional individual-score parity to a dense related family (for example `indiv_score_related_dense_glmmkin_cond`). (Resolved; extracted `baselines/indiv_score_related_dense_glmmkin_cond_nonexample601_sentinels.json` from `../STAAR` and added parity scenario `specs/indiv_score_related_dense_glmmkin_cond_nonexample601.yaml`; parity now `54 passed`)
- STAAR-54: Add at least one additional distinct non-`example` seed baseline (beyond `nonexample601`) for conditional related workflows to reduce single-seed overfitting risk in release gating. (Resolved; extracted `nonexample602` baseline artifacts from `../STAAR` and added conditional related parity scenario `specs/staar_related_sparse_glmmkin_cond_nonexample602.yaml` with sentinels `baselines/related_sparse_glmmkin_cond_nonexample602_sentinels.json`; parity now `55 passed`)
- STAAR-55: Optimize repeated related-GLMM workflow execution by caching related null-model fits for string-addressable datasets while preserving parity behavior. (Resolved; added clone-safe cached related null-model path in `src/pystaar/workflows.py`, added reproducible benchmark script `scripts/run_phase3_opt_55_benchmarks.py`, and measured median speedups `~5.02x` for `staar_related_sparse_glmmkin_pure` and `~1.94x` for `ai_staar_related_sparse_glmmkin_find_weight_pure`; see `benchmarks/phase3_opt_55_comparison.csv`)
- STAAR-56: Optimize repeated related AI workflow execution by caching repeated-call AI result payloads when dataset/metadata inputs are unchanged. (Resolved; added cache-aware related AI path in `src/pystaar/workflows.py`, added regression test `test_related_ai_cache_reuses_ai_staar_for_string_dataset`, and measured targeted repeated-call speedups `~12047x` for `ai_staar_related_sparse_glmmkin` and `~33065x` for `ai_staar_related_sparse_glmmkin_find_weight`; see `benchmarks/phase3_opt_56_comparison.csv`)
