#!/usr/bin/env python3
"""Benchmark STAAR-56 optimization for repeated related AI workflow runs."""
...
SCENARIOS: tuple[Scenario, ...] = (
    Scenario(
        scenario_id="ai_staar_related_sparse_glmmkin",
        function_name="ai_staar_related_sparse_glmmkin",
        kwargs={
            "dataset": "example",
            "seed": 600,
...
MODES = ("baseline_no_ai_result_cache", "optimized_ai_result_cache")
...
def _configure_mode(mode: str, original_ai_staar):
    if mode == "baseline_no_ai_result_cache":
        # _related_ai_common cache requires ai_staar identity to match
        # _ORIGINAL_AI_STAAR. Wrapping bypasses only the result cache.
        def _passthrough_ai_staar(*args, **kwargs):
            return original_ai_staar(*args, **kwargs)

        workflows.ai_staar = _passthrough_ai_staar
        return
    if mode == "optimized_ai_result_cache":
        workflows.ai_staar = original_ai_staar
        return
...
def _build_report(
    ...
    baseline_map = {
        ...
        if row["mode"] == "baseline_no_ai_result_cache"
    }
    optimized_map = {
        row["scenario_id"]: row
        for row in summary_rows
        if row["mode"] == "optimized_ai_result_cache"
    }

    lines: list[str] = []
    lines.append("# Phase 3 Performance Snapshot (STAAR-56)")
    lines.append("")
    ...
    lines.append("")
    lines.append("- `benchmarks/phase3_opt_56_comparison.csv`")
    lines.append("")
    lines.append("## Notes")
    lines.append("")
    lines.append("- Baseline mode bypasses AI result caching by wrapping `workflows.ai_staar` with a passthrough function.")
    lines.append("- Optimized mode restores the original `ai_staar` callable and enables repeated-call AI result caching.")
    lines.append("- Sentinels are expected to match between modes; values are recorded in `benchmarks/phase3_opt_56_meta.json`.")
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    parser.add_argument(
        "--raw-output",
        type=Path,
        default=ROOT / "benchmarks" / "phase3_opt_56_raw.csv",
    )
    parser.add_argument(
        "--summary-output",
        type=Path,
        default=ROOT / "benchmarks" / "phase3_opt_56_summary.csv",
    )
    parser.add_argument(
        "--comparison-output",
        type=Path,
        default=ROOT / "benchmarks" / "phase3_opt_56_comparison.csv",
    )
    parser.add_argument(
        "--meta-output",
        type=Path,
        default=ROOT / "benchmarks" / "phase3_opt_56_meta.json",
    )
    parser.add_argument(
        "--report-output",
        type=Path,
        default=ROOT / "reports" / "performance_opt_56.md",
    )
    ...
    original_ai_staar = workflows.ai_staar
    ...
    try:
        for scenario in SCENARIOS:
            for mode in MODES:
                _configure_mode(mode=mode, original_ai_staar=original_ai_staar)
    ...
    summary_lookup = {(row["mode"], row["scenario_id"]): row for row in summary_rows}
    for scenario in SCENARIOS:
        baseline_sentinel = mode_scenario_sentinel[("baseline_no_ai_result_cache", scenario.scenario_id)]
        optimized_sentinel = mode_scenario_sentinel[("optimized_ai_result_cache", scenario.scenario_id)]
        ...
        baseline = summary_lookup[("baseline_no_ai_result_cache", scenario.scenario_id)]
        optimized = summary_lookup[("optimized_ai_result_cache", scenario.scenario_id)]
        comparison_rows.append(
            {
                "scenario_id": scenario.scenario_id,
                "baseline_no_ai_result_cache_median_seconds": baseline["median_seconds"],
                "optimized_ai_result_cache_median_seconds": optimized["median_seconds"],
                "delta_seconds": optimized["median_seconds"] - baseline["median_seconds"],
                "speedup_vs_baseline_no_ai_result_cache": baseline["median_seconds"]
    ...
    _write_csv(
        ...
        fieldnames=[
            "scenario_id",
            "baseline_no_ai_result_cache_median_seconds",
            "optimized_ai_result_cache_median_seconds",
            "delta_seconds",
            "speedup_vs_baseline_no_ai_result_cache",
        ],
    )
    _write_json(args.meta_output, metadata)
    _build_report(args.report_output, metadata, summary_rows)
    return 0
