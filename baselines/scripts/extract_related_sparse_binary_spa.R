...
jsonlite::write_json(
  sentinels,
  "baselines/related_sparse_binary_spa_sentinels.json",
  pretty = TRUE,
  digits = 15,
  auto_unbox = TRUE
)

utils::write.csv(as.matrix(obj_nullmodel$fitted.values), "data/example_glmmkin_binary_spa_sparse_fitted.csv", row.names = FALSE)
utils::write.csv(as.matrix(obj_nullmodel$scaled.residuals), "data/example_glmmkin_binary_spa_sparse_scaled_residuals.csv", row.names = FALSE)
utils::write.csv(as.matrix(obj_nullmodel$XSigma_i), "data/example_glmmkin_binary_spa_sparse_XW.csv", row.names = FALSE)
utils::write.csv(as.matrix(obj_nullmodel$XXSigma_iX_inv), "data/example_glmmkin_binary_spa_sparse_XXWX_inv.csv", row.names = FALSE)
