...
P <- obj_nullmodel$P
PX_adj <- P %*% X_adj
P_cond <- P - X_adj %*% xtx_inv %*% t(PX_adj) -
  PX_adj %*% xtx_inv %*% t(X_adj) +
  X_adj %*% xtx_inv %*% t(PX_adj) %*% X_adj %*% xtx_inv %*% t(X_adj)
Cov_cond <- t(P_cond %*% G) %*% G
...
jsonlite::write_json(
  sentinels,
  "baselines/related_dense_glmmkin_cond_sentinels.json",
  pretty = TRUE,
  digits = 15,
  auto_unbox = TRUE
)
