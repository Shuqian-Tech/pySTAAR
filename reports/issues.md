...
- STAAR-42: Retire `DEV-001` by tightening related-scenario pure-path tolerances and eliminating shadow-scenario xfails. (Resolved; tightened affected related sentinels to `rtol=3.5e-4`, removed shadow `xfail` markers, parity now `46 passed`)
...
- STAAR-45: Expand parity/performance coverage beyond `dataset=\"example\"` by adding at least one additional representative scenario family with exported R baselines, scenario specs, and CI parity execution on the reference backend. (Open)
- STAAR-46: Optimize `staar_related_sparse_binary_spa` pure path on the reference backend and retain parity gates after each change. Baseline median from post-`STAAR-42` probe: `2.343567s` (`benchmarks/phase3_post42_probe_summary.csv`). (Open)
