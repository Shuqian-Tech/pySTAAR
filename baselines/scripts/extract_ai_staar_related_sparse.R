...
for (b in seq_len(B)) {
  ...
  Sigma_iG1 <- as.matrix(obj_nullmodel$Sigma_i %*% G1_sparse)
  Sigma_iG2 <- as.matrix(obj_nullmodel$Sigma_i %*% G2_sparse)
  tSigma_iX_G1 <- t(Sigma_iX) %*% G1
  tSigma_iX_G2 <- t(Sigma_iX) %*% G2

  cov_1 <- t(Sigma_iG1) %*% G1 - t(tSigma_iX_G1) %*% obj_nullmodel$cov %*% tSigma_iX_G1
  cov_2 <- t(Sigma_iG2) %*% G2 - t(tSigma_iX_G2) %*% obj_nullmodel$cov %*% tSigma_iX_G2
  cov_1 <- (cov_1 + t(cov_1)) / 2
  cov_2 <- (cov_2 + t(cov_2)) / 2

  utils::write.csv(cov_1, sprintf("data/example_ai_cov_sparse_s1_b%d.csv", b), row.names = FALSE)
  utils::write.csv(cov_2, sprintf("data/example_ai_cov_sparse_s2_b%d.csv", b), row.names = FALSE)
}
...
jsonlite::write_json(
  sentinels,
  "baselines/ai_staar_related_sparse_glmmkin_sentinels.json",
  pretty = TRUE,
  digits = 15,
  auto_unbox = TRUE
)
